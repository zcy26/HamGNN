...
# "/data/home/yzhong/EPC_test/MoS2_calculated/MoS2_*/"  # path of the DFT output
scfout_paths = "/public/home/cwzhang/acd47nd21x/zhangchangwei/HamNet/PC/"
...
# natsort sorts a string list with human-friendly order (numbers sorted correctly)
...
for idx, scf_path in enumerate(tqdm(scfout_paths)):  # tqdm creates a progress bar on stdout
    ...
    # read H & S
    # an executable read_openmx will run to create the json
    ...
    with open("./HS.json", 'r') as load_f:
        ...
        #
        # on-site Hamiltonian with n_atoms sub matrices, each of size (n_orb, n_orb) (n_orb may differ from each other)
        Hon = load_dict['Hon'][0]
        ...
        Soff = load_dict['Soff']
        # (n_edges, 3) nbr_shift[i]= shift vector that is added to the position of the destination node of the edge with index i to obtain the position of its neighbor

...
graph_data_path = "/public/home/zhangchengyan/workspace/HamGNN/data_user/perovskite/graph/train/"  # path to save the graph data
read_openmx_path = '.'  # path of a binary executable "read_openmx"
max_SCF_skip = 200 # default is 200
scfout_paths = "/public/home/zhangchengyan/workspace/HamGNN/data_user/perovskite/cspbi3_test/poscar_training/*"   # "/data/home/yzhong/EPC_test/MoS2_calculated/MoS2_*/"  # path of the DFT output
dat_file_name = "CsPbI3.dat"  # name of the dat file for DFT
std_file_name = "log.out"
scfout_file_name = "CsPbI3.scfout"  # name of the scfout file of DFT
...
def calculate_weight(data_set, a="fit"):
    ...
    if a == "fit":  # fit a
        ...
        mean = torch.mean(torch.abs(Hoff_mean_all)).numpy()
        a = popt[1]  # coeff in the weight
        print(f"The target Hamiltonian is used to fit the weights, with a={a}, and rmse/mean={rmse/mean}.")
        # plot the fitting
        fig_name = os.path.join(graph_data_path, "fit_H.png")
        plt.scatter(dist_all, Hoff_mean_all, s=3, label="data")
        plt.xlabel("r/au")
        plt.ylabel(r"$\bar{|H|}_{off}$")
        fit_plot_x = np.linspace(dist_all.min(), dist_all.max(), 200)
        fit_plot_y = exp_fit(fit_plot_x, *popt)
        plt.plot(fit_plot_x, fit_plot_y, label="fit")
        plt.legend()
...
print("first_dir:", scfout_paths[0])
print("last_dir:", scfout_paths[-1])
for idx, scf_path in enumerate(scfout_paths):  # tqdm creates a progress bar on stdout
    print(idx+1)
    # file paths
    ...
    if not os.path.exists("./HS.json"):
        warnings.warn("HS.json not found for {scf_path}!")
        continue
    content = []
    with open("./HS.json", "r") as preprocess_f:
        for i in range(10):
            content.append(next(preprocess_f))  # omit the lines below
    content = "".join(content)
    content = content.rstrip()
    content = content.rstrip(",")
    content += "\n}"
    with open("./HS.json", "w") as write_f:

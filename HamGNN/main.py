...
def build_model(config):
    ...
    # Force
        ...
        if config.setup.GNN_Net.lower() == 'dimenet_triplet':
            output_module = Force(num_edge_features=Gnn_net.hidden_channels,
                                  activation=Gnn_net.act, use_bath_norm=True, bias=True, n_h=3)
        else:
            quit()
    ...
    # scalar_per_atom
        ...
        if config.setup.GNN_Net.lower() == 'dimnet':
            ...
            output_module = scalar('mean', Gnn_net.classification, num_node_features=Gnn_net.atom_fea_len,
                                   n_h=config.representation_nets.cgcnn.n_h)
    ...
    if config.post_processing.post_utility is None:
        ...
        post_utility = EPC_output(representation=Gnn_net, output=output_module,
                                  band_win_min=post_param.band_win_min, band_win_max=post_param.band_win_max)
    else:
...
if __name__ == '__main__':
    ...
    if configure.setup.deterministic:
        pl.seed_everything(configure.setup.random_seed, workers=True)
